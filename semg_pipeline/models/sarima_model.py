"""
semg_pipeline/models/sarima_model.py

SARIMA classical baseline for sEMG anomaly detection.
...
⚠️ PERFORMANCE NOTE
--------------------
SARIMA is extremely slow to fit on large windows (1920 samples per window) and
many windows. Training is therefore limited to a configurable subset of subjects
(SARIMA_MAX_TRAIN_SUBJECTS = 5 by default, Sub01–Sub05).
This is documented as a known limitation in the project report.

The fitted models are persisted as pickle files so they only need to be trained once.
"""
...
# ── SARIMA training subset limit ──────────────────────────────────────────────
# Training all 30 subjects × 16 movements × 9 channels would take days.
# We default to 5 subjects; this is a documented limitation.
...
class SARIMAModel:
    ...
    Usage
    -----
        model = SARIMAModel(channel_names=SEMG_CHANNELS)
        model.fit(train_windows, max_windows_per_channel=200)
    ...
    def fit(self, windows: np.ndarray) -> "SARIMAModel":
        ...
        windows : np.ndarray
            ...
            All N windows from SARIMA_MAX_TRAIN_SUBJECTS subjects should
            be passed; this method subsamples internally.
        ...
        # Subsample windows deterministically (reproducibility)
        ...
        for ch_idx in range(n_channels):
            ...
            # pmdarima fits on a long univariate series, then we score window-by-window
    ...
    def score(self, windows: np.ndarray) -> np.ndarray:
        ...
        For each window, the ARIMA model predicts n_pred = window_size // 2
        steps ahead conditioned on the first half as context. MSE is computed
        between predicted and actual second-half signal.
        ...
        for ch_idx in range(n_channels):
            ...
            for w_idx in range(n_windows):
                ...
                try:
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
                        # Update model state with context, then forecast
